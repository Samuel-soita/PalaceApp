...
import { 
    ...
    History, CheckCircle2, AlertTriangle, TrendingUp 
} from 'lucide-react';
import api from '../lib/api-client';
import DashboardLayout from '../components/layout/DashboardLayout';

/**
 * 🏥 SYSTEM HEALTH DASHBOARD - v2.4.0
 * Pure Enterprise-Grade Telemetry.
 */
export default function HealthDashboard() {
    const { data: healthData, isLoading } = useQuery(['system-health'], async () => {
                ...
                <Grid item xs={12} md={4}>
                    <Card className="holographic-card">
                        <CardContent>
                            ...
                            <Typography variant="h4" fontWeight="1000">{performance.latency}ms</Typography>
                            <Typography variant="caption" color="textSecondary" fontWeight="800">SYSTEM LATENCY</Typography>
                            <LinearProgress 
                                variant="determinate" 
                                value={Math.max(0, 100 - (performance.latency / 5))} 
                    ...
                    <Card className="holographic-card">
                        <CardContent>
                            <Box display="flex" justifyContent="space-between" mb={2}>
                                ...
                                <Typography variant="caption" fontWeight="900" color="var(--cyan)">{performance.syncSuccessRate}%</Typography>
                            </Box>
                            ...
                            <LinearProgress 
                                variant="determinate" 
                                value={performance.syncSuccessRate} 
                    ...
                    <Card className="holographic-card">
                        <CardContent>
                            <Box display="flex" justifyContent="space-between" mb={2}>
                                ...
                                <Typography variant="caption" fontWeight="900" color="var(--purple)">{inventory.status}</Typography>
                            </Box>
                            <Typography variant="h4" fontWeight="1000">{inventory.totalUsers} / {inventory.targetScale}</Typography>
                            <Typography variant="caption" color="textSecondary" fontWeight="800">OPERATIONAL SCALE</Typography>
                            <LinearProgress 
                                variant="determinate" 
                                value={(inventory.totalUsers / inventory.targetScale) * 100} 
                    ...
                    <Card className="holographic-card">
                        <CardContent>
                            ...
                            <List sx={{ width: '100%', bgcolor: 'transparent' }}>
                                {telemetry.recentAudits.map((audit: any, index: number) => (
                                    <ListItem key={audit.id} alignItems="flex-start" sx={{ px: 0, borderBottom: index < telemetry.recentAudits.length - 1 ? '1px solid rgba(255,255,255,0.05)' : 'none' }}>
                                        <ListItemAvatar>
                                            <Avatar sx={{ bgcolor: 'rgba(124,58,237,0.1)', color: 'var(--purple)' }}>
                                                {audit.actionType.startsWith('CREATE') ? <TrendingUp size={18} /> : <ShieldAlert size={18} />}
                                            </Avatar>
                                        </ListItemAvatar>
                                        <ListItemText
                                            primary={
                                                <Box display="flex" justifyContent="space-between">
                                                    ...
                                                    <Typography variant="caption" color="textSecondary">{new Date(audit.createdAt).toLocaleTimeString()}</Typography>
                                                </Box>
                                            }
                                            secondary={
                                                <Box display="flex" alignItems="center" gap={1} mt={0.5}>
                                                    <Chip label={audit.actor.role} size="small" sx={{ height: 14, fontSize: '0.55rem', fontWeight: 800 }} />
                                                    <Typography variant="caption" color="textSecondary" fontWeight={600}>by {audit.actor.name}</Typography>
                                                </Box>
                                            }
                                        />
                                    </ListItem>
                            ...
                            <Box display="flex" flexDirection="column" gap={3}>
                                <Box>
                                    <Box display="flex" justifyContent="space-between" mb={0.5}>
                                        ...
                                        <Typography variant="caption" color="success.main" fontWeight="900">{performance.uptime}</Typography>
                                    </Box>
                                ...
                                <Box>
                                    <Box display="flex" justifyContent="space-between" mb={0.5}>
                                        ...
                                        <Typography variant="caption" fontWeight="900">{telemetry.dailySyncEvents}</Typography>
                                    </Box>
                                ...
                                <Box>
                                    <Box display="flex" justifyContent="space-between" mb={0.5}>
                                        ...
                                        <Typography variant="caption" color="error" fontWeight="900">{(performance.failureRate * 100).toFixed(2)}%</Typography>
                                    </Box>
                                    <LinearProgress variant="determinate" value={performance.failureRate * 100} sx={{ height: 2, borderRadius: 1, '& .MuiLinearProgress-bar': { bgcolor: 'var(--red)' } }} />
                                </Box>
                            </Box>
